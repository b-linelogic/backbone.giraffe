(function() {
  ...
  (function(root, factory) {
    ...
    var $document, $window, Giraffe, _afterInitialize, _setEventBindings, _setEventMapBindings, previousGiraffe;
    if (!$) {
    ...
    /**
     * Singleton template cache only accessible from framework. Keys are
     * template ids, values are the corresponding compiled templates.
     * @property templateCache
     * @type {Object}
     */
    var templateCache = Giraffe.templateCache = {}; // holds compiled templates.

     var getTemplate = function(id){
       return templateCache[id] || function(data){
            return '<div class="alert alert-warning">INVALID TEMPLATE</div>'
          }
     };
	  Giraffe.getTemplate = getTemplate;

	  /**
		* Checks if an object is an instance of jQuery
		* @param obj {Object} The object to check
		* @returns {Boolean} True if the object is an instance of jQuery, false, otherwise
		* @method __isJquery
		* @private
		*/
	  var __isJquery = function(obj){
		  if (jQuery === undefined) return false;
		  return obj instanceof jQuery;
	  }

	  Giraffe.templateFunction = _.template;

	  /**
		* Compile and cache a template for later usage with `getTemplate('name')`.
		* Template pre-compile executes much faster because it converts the strings into functions.
		* You can use any template engine. Examples show how to use Underscore/Lodash, Handlebars, Mushtache and DoT.
		* If not specified, the template function will use the default _.template from either underscore or lodash
		* Additional arguments passed after `templateFunc` will be passed into the template function.
		...
		* @example `cacheTemplate('home', '<div>Hello, {{toWho}}!</div>', Handlebars.compile)`
		* @example `cacheTemplate('home', '<div>Hello, {{=it.toWho || 'World'}}!</div>', DoT.compile, templateOpts, definitions)`  last two arguments will be passed into the template function
		*/
	  var cacheTemplate= function(name,template,compileFunc){
		  "use strict";
		  if(! name) return console.error("Must provide name for the template");
		  var t = "";
		  compileFunc = (_.isFunction(compileFunc))? compileFunc : _.isFunction(Giraffe.templateFunction)? Giraffe.templateFunction : _.template;
		  if(_.isElement(template)){
			  t = $.trim(template.innerHTML);
		  }
		  else if(__isJquery(template)){
			  if(!template.length) console.error("template jQuery selector is empty");
			  t = $.trim(template[0].innerHTML);
		  }
		  else if(_.isString(template)) t = template;
		  ...
		  templateCache[name] = compileFunc.apply(this,args);
	  };

	  Giraffe.cacheTemplate = cacheTemplate;

	  /**
		* Similar to cacheTemplate but caches multiple templates using an Object `{name, template}` or the ID of each template element from a query.
		* Note that if using element or jQuery, the innerHTML is used, not the entire element.
		* This is because it is assumed that the `<template>` tag will be used as the outer element and it will have an ID.
		...
		*/
	  var cacheTemplates = function(templates, compileFunc){
		  compileFunc = (_.isFunction(compileFunc)) ? compileFunc : _.isFunction(Giraffe.templateFunction) ? Giraffe.templateFunction : _.template;
		  if (_.isString(templates)){
			  var t = $(t);
			  if(t.length >0) t.each(function(i,el){
				  var id = el.id;
				  if (!id){
		  ...
		  else if(_.isObject(templates)){
			  _.forEach(templates, function(t, name){
				  var args = [name,t, compileFunc];
				  if(arguments.length > 2) args = args.concat(slice(arguments,2));
				  cacheTemplate.apply(this,args);
			  },this)
		  }
    ...
    Giraffe.View = (function(superClass) {
      ...
      *     });
      ...
      View.prototype._createEventsFromUIElements = function() {
        var eventKey, method, newEventKey, ref;
      ...
      * @param {Array/String} events An array or space-separated string of DOM events to bind to the document.
      ...
      View.setDocumentEvents = function(events, prefix) {
        var attr, event, fn1, i, len, selector;
        if (prefix == null) {
        ...
        fn1 = function(event, attr, selector) {
      ...
      * Equivalent to `Giraffe.View.setDocumentEvents(null)`.
      ...
      View.removeDocumentEvents = function(prefix) {
        var currentEvents, event, i, len, selector;
        if (prefix == null) {
          prefix = Giraffe.View._documentEventPrefix;
        }
        prefix = prefix || '';
      ...
      *
		* - `'cache'`
		* 		- `view.template` is a cached template, previously cached using either Giraffe.cacheTemplate() or Giraffe.cacheTemplates()
      *
          ...
          switch (strategy.toLowerCase()) {
                ...
                if (!this._templateFn) {
                  ...
                  switch (typeof this.template) {
                    case 'string':
                      ...
                      break;
                    case 'function':
                      this._templateFn = (function(_this) {
            ...
            case 'jst':
              ...
              break;
				 ...
				 /* Added to allow for getTemplate function usage */
				 case 'cache':
					 templateStrategy = function(){
						 if (!this.template)
							 return '';
						 if (!this._templateFn){
							 this._templateFn = getTemplate(this.template);
						 }
						 return this._templateFn(this.serialize.apply(this, arguments));
					 };
					 break;
    ...
    * @param {Object} [options]
     */
    Giraffe.Router = (function(superClass) {
      ...
      *     }
      ...
      Router.prototype._bindTriggers = function() {
        var appEvent, fn1, fullNs, ref, route;
        if (!this.triggers) {
        ...
        ref = this.triggers;
        fn1 = (function(_this) {
          return function(route, appEvent, fullNs) {
            var callback;
            if (appEvent.indexOf('-> ') === 0) {
              ...
              callback = function() {
                ...
                return (ref1 = _this.app).trigger.apply(ref1, [appEvent].concat(slice.call(args), [route]));
              };
      ...
      * @param {Object} [any] Optional parameters.
      ...
      Router.prototype.cause = function() {
        var any, appEvent, last, ref, route;
        appEvent = arguments[0], any = 2 <= arguments.length ? slice.call(arguments, 1) : [];
        route = this.getRoute.apply(this, [appEvent].concat(slice.call(any)));
        if (route != null) {
          ...
          return (ref = this.app).trigger.apply(ref, [appEvent].concat(slice.call(any)));
        }
      };
      ...
      * @param {Object} [any] Optional parameter.
      ...
      Router.prototype.getRoute = function() {
        ...
        if (route != null) {
          route = this._reverseHash.apply(this, [route].concat(slice.call(any)));
          if (route) {
            if (Backbone.history._hasPushState) {
              return route;
            } else {
      ...
      Router.prototype._reverseHash = function() {
        ...
        route = arguments[0], any = 2 <= arguments.length ? slice.call(arguments, 1) : [];
        first = any[0];
        if (first == null) {
    ...
    * @param {Object} [options]
     */
    Giraffe.Model = (function(superClass) {
    ...
    * @param {Object} [options]
     */
    Giraffe.Collection = (function(superClass) {
    ...
    * @param {Obj} [opts] Extended along with `defaultOptions` onto `obj` minus `options.omittedOptions`. If `options.omittedOptions` is true, all are omitted.
    ...
    Giraffe.configure = function(obj, opts) {
      var omittedOptions, options, ref, ref1;
      if (!_.isObject(obj)) {
        throw new Error("Giraffe.configure requires an object as the first parameter");
      }
      options = _.extend({}, Giraffe.defaultOptions, (ref = obj.constructor) != null ? ref.defaultOptions : void 0, obj.defaultOptions, opts);
      omittedOptions = (ref1 = options.omittedOptions) != null ? ref1 : obj.omittedOptions;
    ...
    * @param {Any} [args...] Any arguments to be passed to the `fn` and disposal events.
    ...
    Giraffe.dispose = function() {
      ...
      if (typeof this.trigger === "function") {
        this.trigger.apply(this, ['disposing', this].concat(slice.call(args)));
      }
      ...
      if (typeof this.trigger === "function") {
        this.trigger.apply(this, ['disposed', this].concat(slice.call(args)));
      }
    ...
    * @param {Function} [afterFn]
    ...
    Giraffe.wrapFn = function(obj, fnName, beforeFn, afterFn) {
      ...
      return obj[fnName] = function() {
        var args, name1, name2, result;
        args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
        if (beforeFn != null) {
          beforeFn.apply(obj, args);
        }
        if (typeof obj[name1 = 'before' + capFnName] === "function") {
          obj[name1].apply(obj, args);
        }
        result = fn.apply(obj, args);
        if (typeof obj[name2 = 'after' + capFnName] === "function") {
          obj[name2].apply(obj, args);
        }
        if (afterFn != null) {
